  ...
  return (
    <div
      className={`relative flex items-center space-x-4 p-4 cursor-pointer transition rounded-lg ${
        active ? "bg-[#F5F5F5]" : "hover:bg-gray-100"
      }`}
    >
      {active && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-3/4 w-2 bg-[#232323] rounded-r-full"></div>
      )}
      ...
      <span
        className={`text-sm font-medium ${
          active ? "text-[#232323]" : "text-[#B1B1B1]"
        }`}
      >
        {text}
      </span>
